...
import { VideoPlayer, 
  mapStateToProps, 
  mapDispatchToProps } from './VideoPlayer.js';
...
describe('VideoPlayer', () => {
  ...
  it('should fetch the currentGame if no games in store', () => {
    const mockBlankPosts = {};
    wrapper = shallow(<VideoPlayer 
      addSingleGameToStore={mockAddSingleGameToStore}
      games={mockBlankPosts}
      history={mockHistory}
      location={mockLocation}
    />);
    expect(getSingleVideo).toHaveBeenCalled();
    expect(mockAddSingleGameToStore).toHaveBeenCalled();
  });
